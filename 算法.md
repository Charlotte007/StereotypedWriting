...
- 完美/满二叉树
  - 每个节点都有两个子节点
  - 总节点个数，假设 二叉树深度为K，则节点总数 2^K -1; 比如 有4层，则总共有 2^4-1=15 节点
  - 第N层（N > 0, N <= K）的节点总数 2^(N-1), 比如 第4层节点总数，2^(4-1) = 8 节点
...
> fx 表示是否存在公共祖先，fl表示左分支是否存在去 q|p, fr表示右分支是否存在 q|p
...
+ 分类替换当前节点
  ...
  + 当需要删除的节点存在右节点，（右> 根>左），为直接组建为BST，直接使用右节点代替当前节点，当前节点的left节点，
