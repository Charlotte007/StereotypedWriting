...
社区PC站，并没有使用首屏SSR，之后是CSR。而是全部采用了SSR的方式。
...
在controller中会进行请求接口，以及用户的user-agent进行判断，如果是IE浏览器，会进行重定向到老版本。接着就会请求接口（包括了菜单数据的接口, 首页数据的接口，我的关注接口，以及通过cookie判断是否登录isU）并将数据进行组装。然后render对应的page。
